...
classdef OOMMF_sim < hgsetget % subclass hgsetget
 ...
 %   end    
   ...
   function plotDispersionX(obj,varargin)
       ...
       waveVectorScale = 2*pi*linspace(-0.5*0.5,0.5*0.5,size(M,2));
       ...
       dt = 2e-11; % <-------- TODO 
       freqScale = linspace(-0.5/dt,0.5/dt,size(M,1))/1e9; 
   ...
   function plotFFTSliceY(obj,varargin)
       ...
       fftSlice = squeeze(FFTFile.Yy(freqInd,:,params.ySlice,params.zRange(1):params.zRange(2)));
       Amp = abs(fftSlice);
       Phase = angle(fftSlice);
   ...
   function plotFFTIntensity(obj,varargin)
       ...
       zFFT = zFFTFile.Yz(:,:,20:60,1:10);
       Y = mean(mean(mean(zFFT,4),3),2);
       Amp = abs(fftshift(Y));
