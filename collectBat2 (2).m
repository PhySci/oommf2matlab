...
function collectBat2(Yz)
    ...
    %YzFile =  matfile('D:\Micromagnet\OOMMF\proj\TransducerAPL\center\pulse\matlab\YzFFT.mat');
    %Yz = YzFile.Yz;
    ...
    fig2 = figure(2);
        ...
        val = squeeze(mean(mean(Yz(freqScaleInd(1):freqScaleInd(2),:,:,19),2),4));
        val = fftshift(val,1);
        ref = min(val(:));
        dB = 10*log10(val/ref);
        
        imagesc(freqScale(freqScaleInd(1):freqScaleInd(2)),yScale,dB(freqScaleInd(1):freqScaleInd(2),:).');
        axis xy;
        %xlim([0 15]);
        xlabel('Freq, GHz');
