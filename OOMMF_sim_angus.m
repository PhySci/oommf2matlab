...
classdef OOMMF_sim_angus < hgsetget % subclass hgsetget
 ...
 methods
   ...
   function [Mx,My,Mz] = loadMagnetisation(obj,varargin)
     ...
     p.addParamValue('fileExt','ovf',@(x) any(strcmp(x,obj.availableExts)));
     p.parse(varargin{:});
     params = p.Results;
...
% Is this the major memory overhead?
...
%      p.addParamValue('makeFFT',false,@islogical);
   ...
   function plotFFTSlice(obj,varargin)
    p = inputParser;

       p.addParamValue('plane_norm','z',@isstr)
       ...
       switch params.plane_norm
           case 'x'
               iScale = linspace(obj.ymin,obj.ymax,obj.ynodes)/1e-6;
               jScale = linspace(obj.zmin,obj.zmax,obj.znodes)/1e-6;
               params.xRange = params.slice;
           case 'y'
               iScale = linspace(obj.xmin,obj.xmax,obj.xnodes)/1e-6;
               jScale = linspace(obj.zmin,obj.zmax,obj.znodes)/1e-6;
               params.yRange = params.slice;
           case 'z'
               iScale = linspace(obj.xmin,obj.xmax,obj.xnodes)/1e-6;
               jScale = linspace(obj.ymin,obj.ymax,obj.ynodes)/1e-6;
               params.zRange = params.slice;

           otherwise 'problem with plane definition'
       end
...
%        Take the desired components of magnetisation
%        for desired range
       ...
       switch params.proj
           case 'x'
               fftSlice = squeeze(FFTFile.Yx(freqInd,...
                   params.xRange(1):params.xRange(end),...
                   params.yRange(1):params.yRange(end),...
                   params.zRange(1):params.zRange(end)));
           case 'y'
...
%        subplot(2,1,1)
        if strcmp(params.scale,'log')
           ...
           imagesc(jScale,iScale,log10(Amp/ref));
%            cbh =colorbar('EastOutside');
       else
           imagesc(jScale,iScale,Amp,[0 1.05*max(Amp(:))]);
%            cbh = colorbar('EastOutside');
           %cbunits('a.u.');
        end

%         title(['Amplitude of FFT, \nu=' num2str(params.freq) 'GHz, slice ' num2str(params.slice),' proj is',params.proj]);
       axis xy equal;
...
%        cbfreeze;
       ...
       camroll(-90);
       ylim([min(iScale) max(iScale)]);
       xlim([min(jScale) max(jScale)]);
       set(gca,'YAxisLocation','right')
       % plot phase map


%       Get amplitude colour map data
       cdata_amp = print('-RGBImage');
...
%        subplot(2,1,2)
       ...
       imagesc(jScale,iScale,Phase,[-pi pi]);
%        title(['Phase of FFT, \nu=' num2str(params.freq) 'GHz, slice ' num2str(params.slice),' proj is',params.proj ]);

       camroll(-90);

       axis xy equal; colormap(hsv);
%        colorbar('SouthOutside'); %cblabel('rad.');
       xlabel(xLabelStr); ylabel(yLabelStr);
       ylim([min(iScale) max(iScale)]);
       xlim([min(jScale) max(jScale)]);
       set(gca,'YAxisLocation','right')

%        Get phase colour map
       cdata_phase = print('-RGBImage');

%        Combine colour maps and plot new composite figure
        ...
        blue = cdata_phase(:,:,3);
        newred = bright.*red;
        newgreen = bright.*green;
        newblue = bright.*blue;
%         max(newblue,1)

        CDC = im2double(cdata_amp) .* im2double(cdata_phase);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   function plotFFTIntensity(obj,varargin)
       ...
       xlim([0 50]); xlabel('Frequency, GHz');
   ...
   function makeMovie(obj,varargin)
       ...
       jScale = linspace(simParams.ymin,simParams.ymax,simParams.ynodes)/1e-6;
       jScale = jScale(params.yRange);

       fig=figure(1);
       for timeFrame = 1:size(Mz,1)
           Ig = imfilter(squeeze(Mz(timeFrame,:,:)).',G,'circular','same','conv');
           handler = imagesc(xScale,jScale,Ig);
           axis xy;
   ...
   function makeGIF(obj,varargin)
       ...
       p.addParamValue('colourRange',6000);
       p.addParamValue('fName','',@isstr);
       p.addParamValue('background',false);
       p.addParamValue('folder',pwd);
       ...
       MzFile = matfile('My.mat');
       Mz = squeeze(MzFile.My(end-params.timeFrames : end,:,params.yRange,params.zSlice));
       ...
       for timeFrame = 1:size(Mz,1)
           ...
           if (timeFrame == 1)
              imwrite(imind,cm,params.fName,'gif', 'Loopcount',inf);
           else
              imwrite(imind,cm,params.fName,'gif','WriteMode','append');
           end
   ...
   %    useGPU - use GPU (boolean)
       ...
       for chunkInd = 1:chunkAmount
           ...
           disp('Mx');
           Mx = MxFile.Mx(1:arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd);
           if (params.background)
               disp('Substract background');
               for timeInd = 1:arrSize(1)/2
                   Mx(timeInd,:,:,:) = ...
           ...
           disp('Write');
           if mod(arrSize(1)/2,2)
               FFTxFile.Yx(1:floor(0.5*arrSize(1)/2),1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                   tmp((ceil(0.5*int(arrSize(1)/2))+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
               FFTxFile.Yx(ceil(0.5*arrSize(1)/2):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:) =...
                   tmp(1:ceil(0.5*arrSize(1)/2),1:arrSize(2),1:arrSize(3),zStart:zEnd);
           else
               FFTxFile.Yx(1:0.5*arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                   tmp((0.5*arrSize(1)/2+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
               FFTxFile.Yx((0.5*arrSize(1)/2+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                   tmp(1:0.5*arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
           end
           ...
           if true
               ...
               My = MyFile.My(1:arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd);
               if (params.background)
                   disp('Substract background');
                   for timeInd = 1:arrSize(1)/2
                       My(timeInd,:,:,:) = ...
               ...
               disp('Write');
               if mod(arrSize(1)/2,2)
                   FFTyFile.Yy(1:floor(0.5*arrSize(1)/2),1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp((ceil(0.5*arrSize(1)/2)+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
                   FFTyFile.Yy(ceil(0.5*arrSize(1)/2):arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp(1:ceil(0.5*arrSize(1)/2),1:arrSize(2),1:arrSize(3),:);
               else
                   FFTyFile.Yy(1:0.5*arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp((0.5*arrSize(1)/2+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
                   FFTyFile.Yy((0.5*arrSize(1)/2+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp(1:0.5*arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
               end
               ...
               Mz = MzFile.Mz(1:arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd);
               if (params.background)
                   disp('Substract background');
                   for timeInd = 1:arrSize(1)/2
                       Mz(timeInd,:,:,:) = ...
               ...
               disp('Write');
               if mod(arrSize(1)/2,2)
                   FFTzFile.Yz(1:floor(0.5*arrSize(1)/2),1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp((ceil(0.5*arrSize(1)/2)+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
                   FFTzFile.Yz(ceil(0.5*arrSize(1)/2):arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp(1:ceil(0.5*arrSize(1)/2),1:arrSize(2),1:arrSize(3),:);
               else
                   FFTzFile.Yz(1:0.5*arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp((0.5*arrSize(1)/2+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
                   FFTzFile.Yz((0.5*arrSize(1)/2+1):arrSize(1)/2,1:arrSize(2),1:arrSize(3),zStart:zEnd) =...
                       tmp(1:0.5*arrSize(1)/2,1:arrSize(2),1:arrSize(3),:);
               end
           end
       end
   ...
   function plotYFreqMap(obj,varargin)
       ...
       p = inputParser;
       p.addParamValue('freqLimit',[0 20],@isnumeric);
       p.addParamValue('xRange',0,@isnumeric);
       p.addParamValue('yRange',0,@isnumeric);
       p.addParamValue('zRange',0,@isnumeric);
       p.addParamValue('scale','log',@(x) any(strcmp(x,{'log','norm'})));
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%  Generate a set of local curvelinear coordinates:

function [xhat yhat zhat] = make_local_coord_transform(obj)
%     load static magnetisation
    x_mag = matfile('Mx.mat'); y_mag = matfile('My.mat'); z_mag = matfile('Mz.mat');
    mx = x_mag.Mx(1,:,:,:); my = y_mag.My(1,:,:,:); mz = z_mag.Mz(1,:,:,:);

%     size(mx)

    m_static = squeeze(cat(5, mx,my,mz));

%     size(m_static(:,:,:,2))

    [curlmx, curlmy, curlmz] = curl(m_static(:,:,:,1),m_static(:,:,:,2),m_static(:,:,:,3));
    curlm = cat(4, curlmx, curlmy, curlmz);
%     size(curlm)
%     normalise the length of curlm
    magnitude = sqrt(curlmx.^2 + curlmy.^2 + curlmz.^2);

%     size(magnitude)
%     max(curlmx(128,1,:,1))
    curlm_norm = cat(4, curlmx./ magnitude, curlmy./ magnitude, curlmz./ magnitude);
%     size(curlm_norm)
%     curlm_norm(:,1,1,1,1)
%     max(curlm_norm(128,1,:,3))
%     compute cross product of M with CURL(M)
    xhat = cross(m_static, curlm_norm);

    yhat = cross(m_static, xhat);
...
function mprime = use_local_coord_transform(obj)
%     find or generate local coordinates
    disp('generating new coordinates')
    [xhat yhat zhat] = make_local_coord_transform(obj);

%     get a handle on the magnetisation through time
    disp('loading magnetisation history')
    xmag = matfile('Mx.mat'); ymag = matfile('My.mat'); zmag = matfile('Mz.mat');

%     declare arrays for the new magnetisation as function of new
%     coordinates
%     size(xmag.Mx)
    disp('allocating memory for magnetisation in new coordinates..')
    mprime = [];

%     load a single time slice at a time
    for n = 1:length(xmag.Mx)
%         msg = ['time slice = '];
%         disp(msg)
%         n
        mx = squeeze(xmag.Mx(n,:,:,:)); my = squeeze(ymag.My(n,:,:,:)); mz = squeeze(zmag.Mz(n,:,:,:));
        m_static = cat(4, mx,my,mz);
%         get the projection magnetisaiton on new coordinates
        size(xhat)
        size(m_static)
        mxhat = xhat.*m_static; myhat = yhat.*m_static; mzhat = zhat.*m_static;
%         sum the contributions to each new component
%         size(mxhat)
%         size(sum(mxhat(:,:,:,:),4))
        mprime_line = cat(4, sum(mxhat(:,:,:,:),4), sum(myhat(:,:,:,:),4), sum(myhat(:,:,:,:),4));
        mprime = cat(1, mprime, mprime_line);


    end

    save('mprime_file.mat','mprime')
end
